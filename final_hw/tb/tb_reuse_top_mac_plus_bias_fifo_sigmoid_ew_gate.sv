`timescale 1ns/1ps
//======================================================================
// tb_reuse_top_mac_plus_bias_fifo_sigmoid_ew_gate.sv
// Smoke test for the reuse-oriented full path with output gate:
//   shared fabric wrapper -> bias -> sigmoid -> join -> EW update -> gate
//
// Purpose:
//   Verify that after introducing the reusable MAC-fabric hierarchy,
//   the current SSM path still behaves like the original top.
//
// Notes:
//   - Reuses the original data pattern, tile start rhythm, and scoreboard.
//   - Memory init paths are updated for the new hierarchy:
//       dut.u_mac.u_dt_sched.u_wbuf.mem_sim
//       dut.u_mac.u_dt_sched.u_xt.mem_sim
//======================================================================
module tb_reuse_top_mac_plus_bias_fifo_sigmoid_ew_gate;
  ...
  initial begin
    $dumpfile("tb_reuse_top_mac_plus_bias_fifo_sigmoid_ew_gate.vcd");
    $dumpvars(0, tb_reuse_top_mac_plus_bias_fifo_sigmoid_ew_gate);
  end
  ...
  reuse_top_mac_plus_bias_fifo_sigmoid_ew_gate #(
      .TILE_SIZE  (TILE_SIZE),
  ...
  initial begin
    ...
    #5;
    $display("[%0t] init WBUF/XT for reuse hierarchy...", $time);
    for (b_i = 0; b_i < N_BANK; b_i = b_i + 1) begin
      for (addr_i = 0; addr_i < WDEPTH; addr_i = addr_i + 1) begin
        ...
        dut.u_mac.u_dt_sched.u_wbuf.mem_sim[b_i][addr_i] = line_i;
      end
    end

    for (int addr = 0; addr < 64; addr = addr + 1) begin
      dut.u_mac.u_dt_sched.u_xt.mem_sim[addr] = {
        16'((4*addr + 4) << 8),
        16'((4*addr + 3) << 8),
        16'((4*addr + 2) << 8),
        16'((4*addr + 1) << 8)
  ...
  initial begin
    ...
    if (got_y == 0)
      ...
      $display("[%0t] PASS reuse top basic gate outputs got_s=%0d got_y=%0d",
               $time, got_s, got_y);
    $finish;
  end
endmodule
