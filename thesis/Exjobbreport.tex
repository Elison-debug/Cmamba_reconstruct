...
since 6G is expected to support not only ubiquitous communication but also
high-accuracy localization and high-resolution sensing
services~\cite{cite:Bourdoux2020}. Localization is also regarded as a key enabler for
future 6G wireless systems~\cite{cite:Trevlakis2023}. With the development of
location-aware applications such as robotic navigation, emergency healthcare, and smart
transportation, wireless localization is expected to support a wider range of
location-aware services~\cite{cite:Tian2024}. At the same time, real-time indoor
positioning has long remained a challenging problem~\cite{cite:Mogyorosi2022}. In
indoor wireless localization systems, localization performance is affected by
environmental factors such as multipath and non-line-of-sight
propagation~\cite{cite:Yaro2023}.In this context, Massive MIMO is particularly
promising for high-precision indoor localization because large antenna arrays can
provide higher angular resolution and support excellent localization accuracy even
under limited bandwidth \cite{cite:TianICC2023}.

From an algorithmic perspective, traditional localization methods include proximity,
triangulation or trilateration, fingerprint matching, and SLAM~\cite{cite:Tian2024}.
However, conventional signal-processing-based methods often suffer from high
algorithmic complexity and may require base-station array
calibration~\cite{cite:Tian2024}. Machine learning provides an attractive alternative,
since wireless localization can be formulated as a regression task from channel state
information to user positions~\cite{cite:TianICC2023}. ML models can exploit raw
transfer functions, received signal strength, power delay profiles, angular spectra,
and correlation functions as input channel features~\cite{cite:Tian2024}.
Measurement-based studies using the Lund University Massive MIMO testbed show that
spatial covariance matrices can capture angular-domain information, while channel
impulse responses can capture delay-domain information~\cite{cite:TianICC2023}. By
combining angular-domain and delay-domain information, the proposed ML-based pipeline
has achieved centimeter-level localization accuracy in an indoor measurement
scenario~\cite{cite:TianICC2023}. However, as the number of antennas increases, the
size of neural networks increases significantly, which adds complexity to training,
deployment, and inference~\cite{cite:Tian2024}. Therefore, efficient neural
architectures and hardware accelerators are needed for practical Massive-MIMO-based
...
that neural network size increases significantly with the number of antennas in Massive
MIMO systems, making training and fine-tuning more challenging~\cite{cite:Tian2024}.
...
However, most existing ML-based Massive MIMO indoor localization methods rely on FCNNs,
CNNs, KNN, or other conventional machine-learning models to process high-dimensional
...
discussed. As reported in Table~\ref{tab:model_variants}, channel mamba
achieves the lowest test error among the evaluated Mamba-family baselines, but it also
requires a much larger model and longer training time. Vanilla Mamba v1 has a compact
model size, but its test error is clearly worse on LuViRA. In contrast, Slim-Mamba keeps
the test mean error close to channel mamba and the FCNN baseline, while reducing model
size, training time, and hardware cost proxy. This result suggests that the main benefit
for this localization task comes from a lightweight recurrent update rather than from a
large multi-branch fusion backbone.


Figure~\ref{fig:cdf_zoom} further compares the localization-error distribution. The
Slim-Mamba curve remains close to the stronger software baselines in the low-error
region, indicating that the simplified model preserves most of the useful localization
behavior. The spatial error example in Fig.~\ref{fig:grid102_error} shows the error
distribution on grid 102 and is used as a qualitative check that the model error remains
spatially bounded rather than being dominated by isolated large failures.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\linewidth]{cdf_zoom.png}
  \caption{CDF of localization error for the evaluated software models.}
  \label{fig:cdf_zoom}
\end{figure}

\begin{figure}[H]
  \centering
  \includesvg[inkscapelatex=false,width=0.75\linewidth]{grid102_error_heatmap}
...
\(K=2\) both achieve the best test mean error of 0.135~m. Longer windows can reduce the
evaluation error in some cases, but the test error increases as \(K\) grows. This
suggests that long temporal contexts add redundant information for the densely sampled
LuViRA setting and may hurt generalization. Therefore, short-window Slim-Mamba is used
as the software model target for the subsequent hardware implementation.
