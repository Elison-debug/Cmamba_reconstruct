...
\hline
\textbf{Model} & \textbf{Input Features} & \textbf{Params (M)} & \textbf{Train Time / epoch (s)} & \textbf{Latency / sample (ms)} & \textbf{EPE Mean (cm)} & \textbf{HW Cost Proxy} \\
\hline
Channel Mamba      & CIR + Power (+Delta) & -- & -- & -- & -- & High \\
Vanilla Mamba v1   & CIR + Power          & -- & -- & -- & -- & Medium \\
Simplified Mamba   & CIR + Power          & -- & -- & -- & -- & Low \\
\hline
\end{tabular}
\end{table*}
...
Accuracy on LuViRA 
& Baseline. 
& Comparable (fill with measured values). 
