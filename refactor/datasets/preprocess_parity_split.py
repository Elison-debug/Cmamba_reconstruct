...
def main():
    ...
    for mp in mat_paths:
        ...
        role = "train" if ((gid % 2 == 1 and args.odd_even=="odd") or (gid % 2 == 0 and args.odd_even=="even")) else "eval"
        ...
        np.savez(out_path, feats=feats.astype(args.dtype), xy=xy_use.astype(np.float32), ts=ts_use.astype(np.float64),
                 meta=json.dumps(dict(taps=args.taps, input_dim=Din, split=dict(role=role, idx=[0,int(L)], total=int(L)))))
    ...
    np.savez(stats_path, feat_mean=mean, feat_std=std, count=int(count_total), Din=int(Din_ref or mean.shape[0]), std_floor=float(args.std_floor), note="stats over TRAIN parity only")
