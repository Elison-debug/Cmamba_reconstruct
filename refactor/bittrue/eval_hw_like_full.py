...
def _block_forward_cppish(blk: dict, base_dir: Path, x_seq: np.ndarray, lut: _ActLut) -> tuple[np.ndarray, dict]:
    ...
    ssm_out = np.zeros_like(u_act, dtype=np.float32)
    frac_bits = 15
    qmin = -(1 << 15)
    qmax = (1 << 15) - 1
    ...
    y_blk = _conv1d_run_integer_desc(blk["out_proj"], base_dir, ssm_out, ssm_out.shape[0], bits=16)
    x_next = residual + y_blk
    return x_next.astype(np.float32), {
        "x_norm": x_norm,
        "u_act": u_act.astype(np.float32),
        "dt": dt.astype(np.float32),
        "ssm_out": ssm_out.astype(np.float32),
...
def _block_forward_hw_like(blk: dict, base_dir: Path, x_seq: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    uv_q88 = _conv1x1_q88_rne_clamp_tc(in_w, x_norm)
    u_q88 = uv_q88[:, :inner]
    z_q88 = uv_q88[:, inner:]
    ...
    dt_q88 = _conv1x1_q88_rne_clamp_tc(dt_w, u_act_q88_rows.reshape(x_seq.shape[0], inner))
    ...
    y_q88 = _conv1x1_q88_rne_clamp_tc(out_w, gate_y_q88_rows.reshape(x_seq.shape[0], inner))
    y_blk = y_q88.astype(np.float32) / 256.0

    x_next_sum = residual + y_blk
    x_next_q88 = _quant_q88(x_next_sum).astype(np.float32) / 256.0
    return x_next_q88.astype(np.float32), {
        "x_norm": x_norm.astype(np.float32),
        "u_act": (u_act_q88_rows.reshape(x_seq.shape[0], inner).astype(np.float32) / 256.0),
        "dt": (dt_q88.astype(np.float32) / 256.0),
        "ssm_out": (ssm_q88_rows.reshape(x_seq.shape[0], inner).astype(np.float32) / 256.0),
        "y_blk": y_blk.astype(np.float32),
        "x_next": x_next_q88.astype(np.float32),
    }
