...
int main(int argc, char** argv) {
    ...
    if (L.type != "conv1d" || L.kernel_size != 1 || L.stride != 1 || L.padding != 0) {
        std::cerr << "Only pointwise conv1d layers are supported in this tool." << "\n";
        return 4;
    }
    // Load weights/scales
    std::vector<int64_t> shpW, shpB, shpWS; std::vector<float> W, B, WS;
    load_npy_float32(L.weight_path, shpW, W);
    if (!L.bias_path.empty()) load_npy_float32(L.bias_path, shpB, B);
    if (!L.weight_quant.scale_path.empty()) load_npy_float32(L.weight_quant.scale_path, shpWS, WS); else { WS.assign(shpW[0], 1.0f); }
    ...
    if (L.act_quant.scheme != "per_tensor_asym") {
        std::cerr << "This compare tool currently supports only per-tensor-asym activations." << "\n";
        return 6;
    }
    for (int i = 0; i < in_f; ++i) x_q[i] = quant_per_tensor_asym(x[i], L.act_quant.scale_scalar, L.act_quant.zero_point, 0, 255);
    ...
    for (int o = 0; o < out_f; ++o) {
        ...
        double yy = (double)acc * (L.act_quant.scale_scalar * (double)WS[o]);
