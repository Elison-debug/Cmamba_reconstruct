...
def _stage_spec() -> list[dict]:
    return [
        ...
        {
            "name": "reuse_rmsnorm_scheduler",
            "rtl_top": "reuse_rmsnorm_scheduler",
            "status": "planned",
            "description": "Standalone Q8.8 RMSNorm preprocessor for block input h.",
            "golden_inputs": ["h_wr_addr.mem", "h_raw_wr_data_q88.mem", "norm_weight_q88.mem"],
            "golden_outputs": ["h_norm_golden_q88.mem"],
            "notes": [
                "Reads raw block input h from SRAM, computes scalar rms, then applies per-channel norm weight.",
                "Golden matches the integer hardware model used by block-top integration.",
            ],
        },
        {
...
def _div_round_nearest_signed(num: int, den: int) -> int:
    if den <= 0:
...
def _rmsnorm_q88_hw(x_q88: np.ndarray, weight: np.ndarray, eps_q16: int = 1) -> tuple[np.ndarray, np.ndarray, int]:
    x_i = np.asarray(x_q88, dtype=np.int64).reshape(-1)
    w_q88 = _quant_q88(np.asarray(weight, dtype=np.float32).reshape(-1)).astype(np.int64)
    if x_i.shape[0] != w_q88.shape[0]:
        raise ValueError(f"rmsnorm length mismatch: x={x_i.shape[0]} weight={w_q88.shape[0]}")

    sum_sq = int(np.sum(x_i * x_i))
    dim = int(max(1, x_i.shape[0]))
    mean_sq_q16 = (sum_sq + (dim // 2)) // dim
    rms_q88 = int(math.isqrt(max(1, mean_sq_q16 + int(eps_q16))))
    if rms_q88 <= 0:
        rms_q88 = 1

    y = np.empty_like(x_i, dtype=np.int64)
    for i in range(x_i.shape[0]):
        y[i] = _div_round_nearest_signed(int(x_i[i]) * int(w_q88[i]), rms_q88)
    return _clamp_s16_arr(y), w_q88.astype(np.int16), rms_q88
...
def _export_reuse_top_stage(case_dir: Path, model: torch.nn.Module, x_arr: np.ndarray) -> dict:
    ...
    raw_h_q88 = _quant_q88(patch_np)
    norm_weight = blk.norm.weight.detach().cpu().numpy().reshape(-1)
    h_q88, norm_weight_q88, rms_q88 = _rmsnorm_q88_hw(raw_h_q88, norm_weight)
    ...
    _write_mem_matrix_rows_q88(stage_dir / "h_raw_wr_data_q88.mem", raw_h_q88.reshape(32, 4))
    _write_mem_matrix_rows_q88(stage_dir / "norm_weight_q88.mem", norm_weight_q88.reshape(32, 4))
    _write_mem_matrix_rows_q88(stage_dir / "h_norm_golden_q88.mem", h_q88.reshape(32, 4))
    ...
    _write_json(
        stage_dir / "control.json",
        {
            ...
            "notes": [
                ...
                "Compare raw_h->norm->in_proj->u/z SRAM first, then dt/lam/ssm/gate, then y.",
                ...
                f"Norm golden uses integer rms_q88={int(rms_q88)} with per-channel norm_weight_q88.",
    ...
    _write_json(
        stage_dir / "vectors.json",
        {
            ...
            "files": {
                ...
                "h_raw_wr_data_q88": "h_raw_wr_data_q88.mem",
                "norm_weight_q88": "norm_weight_q88.mem",
                "h_norm_golden_q88": "h_norm_golden_q88.mem",
...
def _load_existing_float_cache(out_dir: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    missing = [str(p) for p in (samples_path, y_true_path, y_float_path) if not p.exists()]
    if missing:
        raise FileNotFoundError(
            "reuse_float_only requested, but float cache files are missing: " + ", ".join(missing)
        )
    x_arr = np.load(samples_path)
    y_true_arr = np.load(y_true_path)
    y_float_arr = np.load(y_float_path)
    print(f"[cache] reuse existing float cache from {float_dir} with {x_arr.shape[0]} samples", flush=True)
    return x_arr, y_true_arr, y_float_arr


def _export_reuse_rmsnorm_stage(case_dir: Path, model: torch.nn.Module, x_arr: np.ndarray) -> dict:
    ...
    with torch.inference_mode():
        ...
        if patch.shape[1] != 1:
            return {
                "generated": False,
                "reason": f"reuse_rmsnorm_scheduler currently assumes one patch/token, got num_patches={int(patch.shape[1])}",
            }

    raw_h_np = patch[0, 0].cpu().numpy()
    raw_h_q88 = _quant_q88(raw_h_np)
    norm_weight = blk.norm.weight.detach().cpu().numpy().reshape(-1)
    h_norm_q88, norm_weight_q88, rms_q88 = _rmsnorm_q88_hw(raw_h_q88, norm_weight)

    _write_mem_u16_scalar(stage_dir / "h_wr_addr.mem", list(range(32)), width_hex=2)
    _write_mem_matrix_rows_q88(stage_dir / "h_raw_wr_data_q88.mem", raw_h_q88.reshape(32, 4))
    _write_mem_matrix_rows_q88(stage_dir / "norm_weight_q88.mem", norm_weight_q88.reshape(32, 4))
    _write_mem_matrix_rows_q88(stage_dir / "h_norm_golden_q88.mem", h_norm_q88.reshape(32, 4))
    _write_json(
        stage_dir / "vectors.json",
        {
            "d_model": int(blk.args.d_model),
...
def main() -> None:
    ...
    p.add_argument("--reuse_float_only", action="store_true", help="Reuse out_dir/float/*.npy and skip dataset/model float-cache generation.")
    args = p.parse_args()
    ...
    float_dir = out_dir / "float"
    if args.reuse_float_only:
        x_arr, y_true_arr, y_float_arr = _load_existing_float_cache(out_dir)
    else:
        cfg = TrainConfig(
    ...
    _write_json(
        out_dir / "meta" / "run.json",
        {
            ...
            "reuse_float_only": bool(args.reuse_float_only),
    ...
    for stage in _stage_spec():
        ...
        elif stage["name"] == "reuse_rmsnorm_scheduler":
            manifest["generated_artifacts"] = _export_reuse_rmsnorm_stage(out_dir, model, x_arr)
