...
class MambaOriginBlock(nn.Module):
    """Original Mamba selective scan with dt_rank/d_state support."""
    ...
    def __init__(self, args: ModelArgs) -> None:
        ...
        D = args.d_model
        inner = args.d_inner
        self.dt_rank = int(args.dt_rank)
        self.d_state = int(args.d_state)
        self.use_gate = not bool(args.gate_off)

        use_q_block = bool(args.q_block_linear or args.quantize_all)
        ...
        self.x_proj = nn.Linear(inner, self.dt_rank + 2 * self.d_state, bias=False)
        self.dt_proj = nn.Linear(self.dt_rank, inner, bias=True)

        A = torch.arange(1, self.d_state + 1, dtype=torch.float32).unsqueeze(0).repeat(inner, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.norm(x)
        hp = h.permute(0, 2, 1)
        uv = self.in_proj(hp).permute(0, 2, 1)
        u, z = uv.split(self.args.d_inner, dim=-1)
...
class MambaSlim(nn.Module):
    def __init__(self, args: ModelArgs, block_cls: type[nn.Module] | None = None):
        super().__init__()
        self.args = args
        ...
        block_impl = block_cls or SlimMambaBlock
        self.blocks = nn.ModuleList([block_impl(args) for _ in range(args.n_layer)])
...
class MambaOrigin(MambaSlim):
    """Full Mamba block variant using dt_rank selective scan."""

    def __init__(self, args: ModelArgs):
        super().__init__(args, block_cls=MambaOriginBlock)
