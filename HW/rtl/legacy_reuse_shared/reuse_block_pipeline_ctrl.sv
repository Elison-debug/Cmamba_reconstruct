...
module reuse_block_pipeline_ctrl #(
    ...
    output logic launch_req [0:3],
    input  logic blk_busy   [0:3],
    input  logic blk_done   [0:3],

    input  logic blk_y_valid [0:3],
    output logic blk_y_ready [0:3],
    input  logic signed [DATA_WIDTH-1:0] blk_y_data [0:3][TILE_SIZE-1:0],

    output logic blk_h_wr_en [0:3],
    output logic [4:0] blk_h_wr_addr [0:3],
    output logic signed [DATA_WIDTH-1:0] blk_h_wr_data [0:3][TILE_SIZE-1:0],
    ...
    // Registered inter-block write channel:
    // capture producer fire at block N, then drive a clean 1-cycle
    // h_wr pulse into block N+1 on the next cycle.
    logic pipe_wr_valid;
    logic [1:0] pipe_wr_blk;
    ...
    always_comb begin
        ...
        end else if (INTER_BLOCK_PIPELINE && pipe_wr_valid) begin
    ...
    always_comb begin
        // Guard final output validity with blk3 busy-state to avoid
        // propagating pre-/post-run unknowns from the internal stream wires.
        y_valid = (state == ST_RUN && cur_blk == 2'd3 && blk_busy[3]) ? blk_y_valid[3] : 1'b0;
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
