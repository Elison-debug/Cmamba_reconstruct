...
module reuse_mamba_board_shell_stream #(
    ...
    parameter string STAGE_DIR_B0 = "E:/course/smamba/final_hw/cases/c03/stages/reuse_mamba_block_top_block0",
    parameter string STAGE_DIR_B1 = "E:/course/smamba/final_hw/cases/c03/stages/reuse_mamba_block_top_block1",
    parameter string STAGE_DIR_B2 = "E:/course/smamba/final_hw/cases/c03/stages/reuse_mamba_block_top_block2",
    parameter string STAGE_DIR_B3 = "E:/course/smamba/final_hw/cases/c03/stages/reuse_mamba_block_top_block3"
) (
            ...
            case (ctrl_state)
                ...
                ST_LOAD_WAIT: begin
                    if (preload_h_done) begin
                        frame_loaded <= 1'b1;
                        run_req_wait_cnt <= 4'd8;
                        ctrl_state <= ST_RUN_REQ;
                    end
                end

                ST_RUN_REQ: begin
                    // Keep a deterministic gap after frame preload so all downstream
                    // write-visible states settle before launching the core.
                    if (frame_loaded && !core_busy) begin
                        if (run_req_wait_cnt != 4'd0) begin
                            run_req_wait_cnt <= run_req_wait_cnt - 4'd1;
                        end else begin
