...
module slm_fabric_scheduler #(
    ...
    parameter int ARB_POLICY = 0   // 0: fixed priority, 1: round robin
)(
    input  logic clk,
    input  logic rst_n,
    ...
    typedef enum logic [1:0] { OWN_DT=2'd0, OWN_IN=2'd1, OWN_OUT=2'd2 } owner_t;
    ...
    owner_t grant_owner, last_grant_owner;
    logic sel_dt, sel_in, sel_out;

    always_ff @(posedge clk) begin
        if (!rst_n) begin
            last_grant_owner <= OWN_OUT;
        end else if (fabric_valid_out) begin
            last_grant_owner <= grant_owner;
        end
    end
    ...
    always_comb begin
        ...
        if (ARB_POLICY == 1) begin
            unique case (last_grant_owner)
                OWN_DT: begin
                    if (in_busy) sel_in = 1'b1;
                    else if (out_busy) sel_out = 1'b1;
                    else if (dt_busy) sel_dt = 1'b1;
                end
                OWN_IN: begin
                    if (out_busy) sel_out = 1'b1;
                    else if (dt_busy) sel_dt = 1'b1;
                    else if (in_busy) sel_in = 1'b1;
                end
                default: begin
                    if (dt_busy) sel_dt = 1'b1;
                    else if (in_busy) sel_in = 1'b1;
                    else if (out_busy) sel_out = 1'b1;
                end
            endcase
        ...
        grant_owner = OWN_DT;
        if (sel_in) grant_owner = OWN_IN;
        else if (sel_out) grant_owner = OWN_OUT;
    ...
    reuse_shared_mac_fabric #(
        ...
        .A0_mat(active_A0_mat),
        .A1_mat(active_A1_mat),
        .A2_mat(active_A2_mat),
        .A3_mat(active_A3_mat),
        .B0_mat(active_B0_mat),
        .B1_mat(active_B1_mat),
        .B2_mat(active_B2_mat),
        .B3_mat(active_B3_mat),
