...
module reuse_mamba_block_top #(
    ...
    logic                         inproj_h_wr_en;
    logic [4:0]                   inproj_h_wr_addr;
    logic signed [DATA_WIDTH-1:0] inproj_h_wr_data [TILE_SIZE-1:0];
    logic                         inproj_core_busy;
    logic                         inproj_core_done;
    ...
    assign pcap_start = block_auto_mode ? inproj_core_done : 1'b0;
    assign outproj_start_int = block_auto_mode ? pcap_done : 1'b0;
    assign uact_fill_done = (uact_wr_count == 7'd64);
    assign inproj_busy = block_auto_mode ? (norm_busy || inproj_core_busy) : inproj_core_busy;
    assign inproj_done = block_auto_mode ? inproj_core_done : inproj_core_done;

    always_comb begin
        inproj_h_wr_en   = block_auto_mode ? norm_wr_en : h_wr_en;
        inproj_h_wr_addr = block_auto_mode ? norm_wr_addr : h_wr_addr;
        for (int i = 0; i < TILE_SIZE; i++) begin
            inproj_h_wr_data[i] = block_auto_mode ? norm_wr_data[i] : h_wr_data[i];
        end
    end
                ...
                case (blk_st)
                    BLK_IDLE: begin
                        ...
                        if (block_start) begin
                            norm_start_int <= 1'b1;
                            blk_st <= BLK_START_NORM;
                        end
                    end

                    BLK_START_NORM: begin
                        blk_st <= BLK_WAIT_NORM;
                    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (!block_auto_mode || blk_st == BLK_IDLE || blk_st == BLK_START_NORM || blk_st == BLK_WAIT_NORM ||
                blk_st == BLK_START_INPROJ || blk_st == BLK_WAIT_INPROJ) begin
                uact_wr_count <= '0;
    ...
    reuse_in_proj_scheduler #(
        ...
        .busy(inproj_core_busy),
        .done(inproj_core_done),
        .h_wr_en(inproj_h_wr_en),
        .h_wr_addr(inproj_h_wr_addr),
        .h_wr_data(inproj_h_wr_data),
        .u_rd_en(u_mux_rd_en),
