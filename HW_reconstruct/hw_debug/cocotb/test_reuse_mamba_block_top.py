...
async def test_reuse_mamba_block_top_inproj_partial(dut):
    ...
    dut.gamma_wr_en.value = 0
    dut.gamma_wr_addr.value = 0
    for lane in range(4):
        dut.h_wr_data[lane].value = 0
        dut.gamma_wr_data[lane].value = 0
    ...
    h_data = read_mem_packed_u16(stage_dir / "h_raw_wr_data_q88.mem", signed=True)
    norm_w = read_mem_packed_u16(stage_dir / "norm_weight_q88.mem", signed=True)
    ...
    for addr in range(len(norm_w)):
        await RisingEdge(dut.clk)
        dut.gamma_wr_en.value = 1
        dut.gamma_wr_addr.value = addr
        for lane in range(4):
            dut.gamma_wr_data[lane].value = int(norm_w[addr, lane]) & 0xFFFF
    await RisingEdge(dut.clk)
    dut.gamma_wr_en.value = 0
