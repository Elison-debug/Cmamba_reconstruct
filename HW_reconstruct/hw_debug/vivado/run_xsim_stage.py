...
def _stage_cfg(stage: str) -> dict:
    all_rtl = sorted(str(p) for p in RTL_DIR.glob("*.sv"))
...
def main() -> None:
    ...
    p.add_argument(
        "--stage",
        required=True,
        choices=["sigmoid4_vec", "reuse_top_gate", "reuse_mamba_block_top", "reuse_mamba_block_top_hw_debug"],
    )
    ...
    if args.stage == "reuse_mamba_block_top_hw_debug":
