...
//
//=============================================================================
module tb_reuse_mamba_block_top_sim6_norm;
  ...
  initial begin
    $dumpfile("tb_reuse_mamba_block_top_sim6_norm.vcd");
    $dumpvars(0, tb_reuse_mamba_block_top_sim6_norm);
  end
  ...
  logic                         gamma_wr_en;
  logic [4:0]                   gamma_wr_addr;
  logic signed [DATA_WIDTH-1:0] gamma_wr_data [TILE_SIZE-1:0];
  logic                         u_rd_en;
  logic [5:0]                   u_rd_addr;
  ...
  logic [63:0]  h_raw_wr_data_mem [0:H_DEPTH-1];
  logic [63:0]  norm_weight_mem [0:H_DEPTH-1];
  logic [63:0]  h_norm_golden_mem [0:H_DEPTH-1];
  logic [63:0]  h_norm_calc_mem [0:H_DEPTH-1];
  ...
  logic [63:0]  z_silu_rtl_mem [0:U_DEPTH-1];
  logic [63:0]  dt_rtl_mem [0:U_DEPTH-1];
  logic [63:0]  lam_rtl_mem [0:U_DEPTH-1];
  logic [63:0]  ssm_rtl_mem [0:U_DEPTH-1];
  logic [63:0]  gate_y_rtl_mem [0:U_DEPTH-1];
  int dt_rd_checks;
  int dt_rd_errors;
  int xt_checks;
  int xt_errors;
  ...
  int p_stream_ok;
  int p_stream_idx;
  reg p_stream_first_mismatch_seen;
  int y_stream_errors;
  int y_stream_ok;
  int y_stream_idx;
  reg y_stream_first_mismatch_seen;
  int y_mem_errors;
  int outproj_dbg_count;
  int z_silu_errors;
  int z_silu_ok;
  int z_silu_idx;
  reg z_silu_first_mismatch_seen;
  int dt_stream_errors;
  int dt_stream_ok;
  int dt_stream_idx;
  reg dt_stream_first_mismatch_seen;
  int lam_stream_errors;
  int lam_stream_ok;
  int lam_stream_idx;
  reg lam_stream_first_mismatch_seen;
  int ssm_stream_errors;
  int ssm_stream_ok;
  int ssm_stream_idx;
  reg ssm_stream_first_mismatch_seen;
  int gate_stream_errors;
  int gate_stream_ok;
  int gate_stream_idx;
  reg gate_stream_first_mismatch_seen;
  ...
  task automatic ensure_dump_dirs();
    string mk_root_cmd;
    string mk_stage_cmd;
    int status_root;
    int status_stage;
    begin
      rtl_root_dir  = join_path(case_dir, "rtl_out");
      rtl_stage_dir = join_path(rtl_root_dir, "reuse_mamba_block_top");
      mk_root_cmd   = {"cmd /c if not exist \"", rtl_root_dir, "\" mkdir \"", rtl_root_dir, "\""};
      mk_stage_cmd  = {"cmd /c if not exist \"", rtl_stage_dir, "\" mkdir \"", rtl_stage_dir, "\""};
      status_root   = $system(mk_root_cmd);
      status_stage  = $system(mk_stage_cmd);
      if (status_root != 0)
        $fatal(1, "failed to create rtl dump root dir: %s status=%0d", rtl_root_dir, status_root);
      if (status_stage != 0)
        $fatal(1, "failed to create rtl dump stage dir: %s status=%0d", rtl_stage_dir, status_stage);
      $display("[%0t] RTL_OUT_DIR=%s", $time, rtl_stage_dir);
    end
  endtask

  task automatic dump_mem64_file(
      input string path,
      input logic [63:0] mem[],
      input integer depth
  );
    integer fd;
    begin
      fd = $fopen(path, "w");
      if (fd == 0)
        $fatal(1, "failed to open dump file: %s", path);
      for (int idx = 0; idx < depth; idx++)
        $fdisplay(fd, "%016h", mem[idx]);
      $fclose(fd);
      $display("[%0t] dumped %0d rows -> %s", $time, depth, path);
    end
  endtask

  task automatic dump_stage_outputs();
    logic [63:0] h_norm_rtl_mem [0:H_DEPTH-1];
    logic [63:0] u_rtl_mem      [0:U_DEPTH-1];
    logic [63:0] z_rtl_mem      [0:U_DEPTH-1];
    logic [63:0] u_act_rtl_mem  [0:U_DEPTH-1];
    logic [63:0] y_rtl_mem      [0:Y_DEPTH-1];
    begin
      for (int addr = 0; addr < H_DEPTH; addr++) begin
        h_norm_rtl_mem[addr] = dut.u_in_proj.u_h_sram.mem_sim[addr];
      end
      for (int addr = 0; addr < U_DEPTH; addr++) begin
        u_rtl_mem[addr]     = dut.u_in_proj.u_u_sram.mem_sim[addr];
        z_rtl_mem[addr]     = dut.u_in_proj.u_z_sram.mem_sim[addr];
        u_act_rtl_mem[addr] = dut.u_uact_sram.mem_sim[addr];
      end
      for (int addr = 0; addr < Y_DEPTH; addr++) begin
        y_rtl_mem[addr] = dut.u_out_proj.u_y_sram.mem_sim[addr];
      end

      dump_mem64_file(join_path(rtl_stage_dir, "h_norm_golden_q88.mem"),   h_norm_rtl_mem, H_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "u_golden_q88.mem"),        u_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "z_golden_q88.mem"),        z_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "u_act_golden_q88.mem"),    u_act_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "z_silu_golden_q88.mem"),   z_silu_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "dt_golden_q88.mem"),       dt_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "lam_golden_q016.mem"),     lam_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "ssm_golden_q88.mem"),      ssm_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "gate_y_golden_q88.mem"),   gate_y_rtl_mem, U_DEPTH);
      dump_mem64_file(join_path(rtl_stage_dir, "y_golden_q88.mem"),        y_rtl_mem, Y_DEPTH);
    end
  endtask

  function automatic signed [DATA_WIDTH-1:0] unpack_lane64(
      input [63:0] packed_word,
      input integer lane
  );
    begin
  ...
  task automatic load_case_files();
    ...
    begin
      ...
      $readmemh(join_path(stage_dir, "h_raw_wr_data_q88.mem"), h_raw_wr_data_mem);
      $readmemh(join_path(stage_dir, "norm_weight_q88.mem"), norm_weight_mem);
      $readmemh(join_path(stage_dir, "h_norm_golden_q88.mem"), h_norm_golden_mem);
  ...
  task automatic init_weight_mems_from_case();
    begin
      for (int addr = 0; addr < H_DEPTH; addr++) begin
        @(posedge clk);
        gamma_wr_en   <= 1'b1;
        gamma_wr_addr <= addr[4:0];
        for (int lane = 0; lane < TILE_SIZE; lane++)
          gamma_wr_data[lane] <= unpack_lane64(norm_weight_mem[addr], lane);
      end
      @(posedge clk);
      gamma_wr_en <= 1'b0;
      ...
      $display("[%0t] FILE  norm gamma addr0 = %h", $time, norm_weight_mem[0]);
      $display("[%0t] SRAM  norm gamma addr0 = %h", $time, dut.u_norm.u_gamma_mem.mem_sim[0]);
  ...
  task automatic write_h_from_case();
    begin
      for (int idx = 0; idx < H_DEPTH; idx++) begin
        ...
        for (int lane = 0; lane < TILE_SIZE; lane++)
          h_wr_data[lane] <= unpack_lane64(h_raw_wr_data_mem[idx], lane);
      end
      @(posedge clk);
      h_wr_en   <= 1'b0;
      h_wr_addr <= '0;
      ...
      $display(
          ...
          $time,
          h_wr_addr_mem[0][4:0],
          unpack_lane64(h_raw_wr_data_mem[0], 0),
          unpack_lane64(h_raw_wr_data_mem[0], 1),
          unpack_lane64(h_raw_wr_data_mem[0], 2),
          unpack_lane64(h_raw_wr_data_mem[0], 3)
      );
      $display(
          "[%0t] HRAW sram row0 data=%0d,%0d,%0d,%0d",
          $time,
          $signed(dut.u_norm.u_raw_h_sram.mem_sim[0][0*DATA_WIDTH +: DATA_WIDTH]),
          $signed(dut.u_norm.u_raw_h_sram.mem_sim[0][1*DATA_WIDTH +: DATA_WIDTH]),
          $signed(dut.u_norm.u_raw_h_sram.mem_sim[0][2*DATA_WIDTH +: DATA_WIDTH]),
          $signed(dut.u_norm.u_raw_h_sram.mem_sim[0][3*DATA_WIDTH +: DATA_WIDTH])
      );
    end
  endtask

  task automatic compare_norm_mem();
    reg [63:0] sum_sq;
    ...
    reg signed [15:0] got_h;
    reg signed [15:0] exp_h;
    int ok;
    reg first_mismatch_seen;
    begin
      sum_sq = 0;
      ok = 0;
      ...
      for (int addr = 0; addr < H_DEPTH; addr++) begin
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          x_lane = unpack_lane64(h_raw_wr_data_mem[addr], lane);
          lane_sq = x_lane;
      ...
      if (rms_q88 == 0)
        rms_q88 = 1;
      $display("[%0t] NORM rms_q88=%0d", $time, rms_q88);

      for (int addr = 0; addr < H_DEPTH; addr++) begin
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          x_lane = unpack_lane64(h_raw_wr_data_mem[addr], lane);
          g_lane = unpack_lane64(norm_weight_mem[addr], lane);
          y_lane[lane] = clamp_s16(div_round_nearest_signed($signed(x_lane) * $signed(g_lane), rms_q88[31:0]));
        end
        h_norm_calc_mem[addr] = pack4x16(y_lane[0], y_lane[1], y_lane[2], y_lane[3]);
      end

      $display(
          "[%0t] NORM golden row0=%0d,%0d,%0d,%0d",
          $time,
          unpack_lane64(h_norm_calc_mem[0], 0),
          unpack_lane64(h_norm_calc_mem[0], 1),
          unpack_lane64(h_norm_calc_mem[0], 2),
          unpack_lane64(h_norm_calc_mem[0], 3)
      );
      $display(
          "[%0t] NORM file   row0=%0d,%0d,%0d,%0d",
          $time,
          unpack_lane64(h_norm_golden_mem[0], 0),
          unpack_lane64(h_norm_golden_mem[0], 1),
          unpack_lane64(h_norm_golden_mem[0], 2),
          unpack_lane64(h_norm_golden_mem[0], 3)
      );
      $display(
          "[%0t] NORM act    row0=%0d,%0d,%0d,%0d",
          $time,
      ...
      for (int addr = 0; addr < H_DEPTH; addr++) begin
        if (h_norm_calc_mem[addr] !== h_norm_golden_mem[addr]) begin
          $fatal(1, "[%0t] norm exported golden mismatch addr=%0d calc=%h file=%h", $time, addr, h_norm_calc_mem[addr], h_norm_golden_mem[addr]);
        end
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          got_h = $signed(dut.u_in_proj.u_h_sram.mem_sim[addr][lane*DATA_WIDTH +: DATA_WIDTH]);
          exp_h = unpack_lane64(h_norm_calc_mem[addr], lane);
          if (got_h !== exp_h) begin
            if (!first_mismatch_seen) begin
              $error("[%0t] FIRST norm SRAM mismatch addr=%0d lane=%0d got=%0d exp=%0d", $time, addr, lane, got_h, exp_h);
              $display(
                  "[%0t] NORM exp row%0d=%0d,%0d,%0d,%0d",
                  $time,
                  addr,
                  unpack_lane64(h_norm_calc_mem[addr], 0),
                  unpack_lane64(h_norm_calc_mem[addr], 1),
                  unpack_lane64(h_norm_calc_mem[addr], 2),
                  unpack_lane64(h_norm_calc_mem[addr], 3)
              );
              $display(
                  "[%0t] NORM act row%0d=%0d,%0d,%0d,%0d",
                  $time,
                  addr,
                  $signed(dut.u_in_proj.u_h_sram.mem_sim[addr][0*DATA_WIDTH +: DATA_WIDTH]),
                  $signed(dut.u_in_proj.u_h_sram.mem_sim[addr][1*DATA_WIDTH +: DATA_WIDTH]),
                  $signed(dut.u_in_proj.u_h_sram.mem_sim[addr][2*DATA_WIDTH +: DATA_WIDTH]),
                  $signed(dut.u_in_proj.u_h_sram.mem_sim[addr][3*DATA_WIDTH +: DATA_WIDTH])
              );
              $display(
                  "[%0t] NORM file row%0d=%0d,%0d,%0d,%0d",
                  $time,
                  addr,
                  unpack_lane64(h_norm_golden_mem[addr], 0),
                  unpack_lane64(h_norm_golden_mem[addr], 1),
                  unpack_lane64(h_norm_golden_mem[addr], 2),
                  unpack_lane64(h_norm_golden_mem[addr], 3)
              );
      ...
      $display("[%0t] PASS norm compare exp/file/act lanes=%0d", $time, ok);
    end
  endtask
  ...
  task automatic compare_uz_mem();
    int u_ok;
    int z_ok;
    reg first_u_mismatch_seen;
    reg first_z_mismatch_seen;
  ...
  task automatic compare_y_mem();
    ...
    begin
      ...
      $display(
          ...
          $time,
          $signed(dut.u_out_proj.u_y_sram.mem_sim[0][0*DATA_WIDTH +: DATA_WIDTH]),
          $signed(dut.u_out_proj.u_y_sram.mem_sim[0][1*DATA_WIDTH +: DATA_WIDTH]),
          $signed(dut.u_out_proj.u_y_sram.mem_sim[0][2*DATA_WIDTH +: DATA_WIDTH]),
          $signed(dut.u_out_proj.u_y_sram.mem_sim[0][3*DATA_WIDTH +: DATA_WIDTH])
      );
      for (int addr = 0; addr < Y_DEPTH; addr++) begin
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          got_y = $signed(dut.u_out_proj.u_y_sram.mem_sim[addr][lane*DATA_WIDTH +: DATA_WIDTH]);
  ...
  int dt_req_addr_q[$];
  reg dt_first_rd_mismatch_seen;
  reg xt_first_mismatch_seen;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (int idx = 0; idx < U_DEPTH; idx++) begin
        z_silu_rtl_mem[idx] <= '0;
        dt_rtl_mem[idx]     <= '0;
        lam_rtl_mem[idx]    <= '0;
        ssm_rtl_mem[idx]    <= '0;
        gate_y_rtl_mem[idx] <= '0;
      end
      dt_stream_errors <= 0;
      dt_stream_ok <= 0;
      dt_stream_idx <= 0;
      ...
      if (dut.p_wr_en && p_stream_idx < U_DEPTH) begin
        gate_y_rtl_mem[p_stream_idx] <= pack4x16(
            dut.p_wr_data[0], dut.p_wr_data[1], dut.p_wr_data[2], dut.p_wr_data[3]);
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          logic signed [DATA_WIDTH-1:0] got_v;
          logic signed [DATA_WIDTH-1:0] exp_v;
      ...
      if (dut.silu_valid && dut.silu_ready && z_silu_idx < U_DEPTH) begin
        z_silu_rtl_mem[z_silu_idx] <= pack4x16(
            dut.silu_vec[0], dut.silu_vec[1], dut.silu_vec[2], dut.silu_vec[3]);
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          logic signed [DATA_WIDTH-1:0] got_v;
          logic signed [DATA_WIDTH-1:0] exp_v;
      ...
      if (dut.dt_mac_valid && dut.dt_mac_ready && dt_stream_idx < U_DEPTH) begin
        dt_rtl_mem[dt_stream_idx] <= pack4x16(
            dut.dt_mac_vec[0], dut.dt_mac_vec[1], dut.dt_mac_vec[2], dut.dt_mac_vec[3]);
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          logic signed [DATA_WIDTH-1:0] got_v;
          logic signed [DATA_WIDTH-1:0] exp_v;
      ...
      if (dut.u_ssm_core.sigmoid_out_valid && dut.u_ssm_core.sigmoid_out_ready && lam_stream_idx < U_DEPTH) begin
        lam_rtl_mem[lam_stream_idx] <= {
            dut.u_ssm_core.sigmoid_out_vec[3],
            dut.u_ssm_core.sigmoid_out_vec[2],
            dut.u_ssm_core.sigmoid_out_vec[1],
      ...
      if (dut.u_ssm_core.s_out_valid && dut.u_ssm_core.s_out_ready && ssm_stream_idx < U_DEPTH) begin
        ssm_rtl_mem[ssm_stream_idx] <= pack4x16(
            dut.u_ssm_core.s_out_vec[0], dut.u_ssm_core.s_out_vec[1],
            dut.u_ssm_core.s_out_vec[2], dut.u_ssm_core.s_out_vec[3]);
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          logic signed [DATA_WIDTH-1:0] got_v;
      ...
      if (dut.ssm_p_valid && dut.ssm_p_ready && gate_stream_idx < U_DEPTH) begin
        gate_y_rtl_mem[gate_stream_idx] <= pack4x16(
            dut.ssm_p_data[0], dut.ssm_p_data[1], dut.ssm_p_data[2], dut.ssm_p_data[3]);
        for (int lane = 0; lane < TILE_SIZE; lane++) begin
          logic signed [DATA_WIDTH-1:0] got_v;
          logic signed [DATA_WIDTH-1:0] exp_v;
  ...
  initial begin
    ...
    dump_stage_outputs();
    $display("[%0t] PASS tb_reuse_mamba_block_top_sim6_norm full compare incl norm", $time);
    $finish;
  end
endmodule
